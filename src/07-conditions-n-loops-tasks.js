...
function doRectanglesOverlap(rect1, rect2) {
  const overLapY = rect2.top < (rect1.top + rect1.height);
  const overLapX = rect2.left < (rect1.left + rect1.width);
...
function findFirstSingleChar(str) {
  for (let i = 0; i < str.length; i += 1) {
    if (str.indexOf(str[i]) === i && (str.slice(i + 1, str.length)).indexOf(str[i]) === -1) {
      return str[i];
    }
  }
  return null;
...
function isCreditCardNumber(ccn) {
  const cardNumber = Array.from(ccn.toString()).reverse().map((value) => +value);
  let sum = 0;
...
function getDigitalRoot(num) {
  const root = num.toString().split('').map((value) => +value).reduce((acc, val) => +acc + val, 0);
  if (root > 9) {
