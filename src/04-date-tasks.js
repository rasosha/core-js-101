...
function timeSpanToString(startDate, endDate) {
  return new Date(new Date(endDate) - new Date(startDate)).toISOString().match(/\d\d:\d\d:\d\d.\d\d\d/gm);
}
...
function angleBetweenClockHands(date) {
  ...
  const hoursAngle = (360 / 12) * (hours % 12) + (minutes / 60) * 30;
  const minutesAngle = (360 / 60) * minutes;
  const angle = Math.abs(hoursAngle - minutesAngle);
  const realAngle = (angle <= 180 ? angle : angle % 180);
  return ((Math.PI * realAngle) / 180);
}
